...
namespace RapidBlockModeExample
{
    class RapidBlockModeExampleCallback : IDisposable
    {
        ...
        public StandardDriverStatusCode RunRapidBlockModeExample(short handle, DeviceResolution resolution)
        {
            ...
            var status = psospaDevice.InitializeChannelsAndRanges(handle, in _channelSettings, numChannels, out NoEnablechannels);
            ...
            ulong nMaxSamples = 0;
            status = DriverImports.psospa.MemorySegments(handle, memorysegments, out nMaxSamples);
            if (status != StandardDriverStatusCode.Ok)
                return status;

            //ps6000aSetNoOfCaptures
            status = DriverImports.psospa.SetNoOfCaptures(handle, memorysegments);
            if (status != StandardDriverStatusCode.Ok)
                return status;
            else
            ...
            status = DriverImports.psospa.NearestSampleIntervalStateless(handle, EnabledChannelsAndPorts_Flags,
                                                                          idealTimeInterval,
                                                                          roundFaster,
                                                                          resolution,
                                                                          out timebase,
            ...
            status = DriverImports.psospa.SetSimpleTrigger(handle, 1, Channel.ChannelA, 1000, TriggerDirection.Rising, 0, 5000000);
            if (status != StandardDriverStatusCode.Ok)
            {
            ...
            status = DriverImports.psospa.RunBlock(handle, noOfPreTriggerSamples, numSamples - noOfPreTriggerSamples,
            ...
            status = DriverImports.psospa.GetNoOfCaptures(handle, out ulong NumofCaptures);
            if (status != StandardDriverStatusCode.Ok)
            {
            ...
            status = DriverImports.psospa.GetValuesBulk(handle, 0, ref numSamples,//segment sample indexs - All (from -> to)
