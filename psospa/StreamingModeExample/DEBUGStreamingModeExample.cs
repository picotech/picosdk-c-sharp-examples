...
namespace StreamingModeExample
{
    ...
    class StreamingModeExample
  {
        ...
        {
            //Define sampling Settings ( Currently - 1 Second per BufferSet (1us x 1MS) )
            ulong numSamples = 100000;//1MS
            ...
            //Set a trigger
            //NOTE the trigger cannot be rearmed during streaming! Capture must be restarted!
            Console.WriteLine("\nSetting Trigger...");
            Console.WriteLine("ChA, Rising edge, threshold @1000 ADC count, AutoTrigger 5 seconds");
            status = DriverImports.PS6000a.SetSimpleTrigger(handle, 1, Channel.ChannelA, 1000, TriggerDirection.Rising, 0, 5000000);//5000000
            if (status != StandardDriverStatusCode.Ok)
            {
            ...
            if (status == StandardDriverStatusCode.Ok)
            {
                ...
                while (i < memorysegments) //loop for each buffer Set created
                {
                    ...
                    {
                        Console.WriteLine("\nBufferSet ready to process: " + i);
                        Console.WriteLine("Channel: " + streamingDataInfoArray[i, 0].Channel);
                        Console.WriteLine("NoOfSamples: " + streamingDataInfoArray[i, 0].NoOfSamples);
                        Console.WriteLine("BufferIndex: " + streamingDataInfoArray[i, 0].BufferIndex);
                        Console.WriteLine("StartIndex: " + streamingDataInfoArray[i, 0].StartIndex);
                        Console.WriteLine("Overflow: " + streamingDataInfoArray[i, 0].Overflow);

                        Console.WriteLine("Triggered: " + streamingDataTriggerInfoArray[i].Triggered);
                        Console.WriteLine("Triggered at sample: " + streamingDataTriggerInfoArray[i].TriggerAt);
                        Console.WriteLine("AutoStop: " + streamingDataTriggerInfoArray[i].AutoStop);
                        Console.WriteLine(" ");
                ...
                ps6000aDevice.WriteArrayToFiles(values, _channelSettings,
                                                ...
                                                "Streaming BufferSet",
                                                (short)noOfPreTriggerSamples);//Trigger point in first BufferSet
                Console.WriteLine("\n");
            }
            return status;
    }
